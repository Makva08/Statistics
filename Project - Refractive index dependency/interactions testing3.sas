...
proc glm data=film_data;
   ...
   model refractive_index = RF1|RF2 Pressure;
   lsmeans RF1 RF2 Pressure/cl pdiff=all adjust=tukey;
run; 

proc glm data=film_data;
   class RF1 RF2 Pressure;
   model refractive_index = RF1 RF2 Pressure thickness RF1*RF2 RF1*Pressure;
   output out=resids p=predicted r=residuals;
run;

proc univariate data=resids;
   histogram residuals / normal; /* Generates a histogram with a normal curve overlay */
   inset n mean std / pos=ne; /* Adds inset with summary statistics to the histogram plot */
   qqplot residuals / normal(mu=est sigma=est); /* Generates a Q-Q plot */
run;

/* To generate matrix of all scatter plots: */
ods graphics on;
proc sgscatter data=film_data;
  matrix RF1 RF2 Pressure / group=thickness;
  title "Matrix of Scatter Plots for RF1, RF2, and Pressure by Thickness";
run;
ods graphics off;
